...
class PatientMatchService
{
    ...
    public function suggestMatches(array $requestData, int $limit = 10): Collection
    {
        ...
        $candidates = DB::table('patients')
            ...
            ->where(function ($query) use ($mobile, $email, $firstName, $lastName, $fullName, $birthDate) {
                if ($mobile !== '') {
                ...
                if ($fullName !== '') {
                    $query->orWhereRaw("LOWER(CONCAT(COALESCE(first_name, ''), ' ', COALESCE(last_name, ''))) LIKE ?", ['%' . strtolower($fullName) . '%']);
                }
