    ...
    <main id="mainContent"
        ...
        <div class="mb-8 flex flex-wrap items-start justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Manage admin, staff, and patient profiles in one place.</p>
            </div>
                ...
                @forelse($admins as $user)
                    <article class="relative rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                        ...
                        <div class="mb-5 flex items-center gap-3">
                            <div class="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-emerald-500 text-base font-bold text-white">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4">
                @forelse($staffs as $user)
                    <article class="relative rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                        ...
                        <div class="mb-5 flex items-center gap-3">
                            <div class="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-blue-500 text-base font-bold text-white">
                                @if (!empty($user->profile_picture))
                                    <img src="{{ asset('storage/' . $user->profile_picture) . '?v=' . urlencode((string) strtotime((string) $user->updated_at)) }}"
                                        alt="{{ $user->username }} profile picture" class="h-full w-full object-cover">
                                @else
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4">
                @forelse($normalUsers as $user)
                    <article class="relative rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                        ...
                        <div class="mb-5 flex items-center gap-3">
                            <div class="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-sky-500 text-base font-bold text-white">
                                @if (!empty($user->profile_picture))
                                    <img src="{{ asset('storage/' . $user->profile_picture) . '?v=' . urlencode((string) strtotime((string) $user->updated_at)) }}"
                                        alt="{{ $user->username }} profile picture" class="h-full w-full object-cover">
                                @else
