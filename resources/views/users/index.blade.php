    ...
    <div id="users-wrap" style="font-family:'Montserrat',sans-serif; -webkit-font-smoothing:antialiased;">
        ...
        <div class="mb-6 border border-[#e4eff8] bg-white">
            <div class="flex flex-wrap items-center justify-between gap-4 px-6 py-6 md:px-8">
                <div>
                    <h1 class="text-[1.35rem] font-extrabold leading-[1.1] tracking-[-.02em] text-[#1a2e3b]">User Accounts
                    </h1>
                    <p class="mt-1 text-[.8rem] text-[#7a9db5]">Manage admin, dentist, and staff profiles, permissions, and
                        lifecycle actions.</p>
                </div>
        ...
        <section class="mb-6 border border-[#e4eff8] bg-white shadow-[0_20px_48px_rgba(0,134,218,.07)]">
            <div class="flex items-center gap-3 border-b border-[#e4eff8] px-6 py-5 md:px-8">
                ...
                <span
                    class="border border-[#d4e8f5] bg-[#f0f8fe] px-3 py-1 text-[.65rem] font-bold uppercase tracking-[.14em] text-[#0086da]">
            ...
            <div class="p-6 md:p-8">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    @forelse($admins as $user)
                        <article
                            class="relative border border-[#e4eff8] bg-white transition hover:shadow-[0_8px_24px_rgba(0,134,218,.1)] hover:-translate-y-0.5">
                            <div class="p-5">
                                <div class="mb-4 flex items-center gap-3">
                                    <div
                                        class="flex h-11 w-11 shrink-0 items-center justify-center bg-[#0086da] text-base font-bold text-white">
                                        {{ strtoupper(substr($user->username, 0, 1)) }}
                                    </div>
                                    <div class="min-w-0">
                                        <h3 class="truncate text-[.88rem] font-extrabold text-[#1a2e3b]">{{ $user->username }}
                                        </h3>
                                        <p class="truncate text-[.72rem] text-[#7a9db5]">ID: #{{ $user->id }}</p>
                                    </div>
                                </div>
                                <div class="mb-1 text-[.72rem] text-[#7a9db5]">
                                    <span class="font-semibold text-[#3d5a6e]">Joined</span>
                            ...
                            <div class="flex items-center gap-[2px] border-t border-[#e4eff8] bg-[#f8fbfe]">
                                ...
                                @if ($user->id !== auth()->id())
                                    <form action="{{ route('users.destroy', $user->id) }}" method="POST"
                                        class="flex-1 border-l border-[#e4eff8]"
                    ...
                    @empty
                        <div
                            class="col-span-full border border-dashed border-[#d4e8f5] bg-[#f8fbfe] px-4 py-10 text-center text-[.82rem] text-[#7a9db5]">
                            No admins found.
                        </div>
                    @endforelse
                </div>
        ...
        <section class="mb-6 border border-[#e4eff8] bg-white shadow-[0_20px_48px_rgba(0,134,218,.07)]">
            ...
            <div class="p-6 md:p-8">
                <p class="mb-6 text-[.78rem] text-[#7a9db5]">Clinical users focused on appointments, queue flow, and patient
                    care.</p>
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    @forelse($dentists as $user)
                        <article
                            ...
                            <div class="p-5">
                                <div class="mb-4 flex items-center gap-3">
                                    <div
                                        class="flex h-11 w-11 shrink-0 items-center justify-center bg-[#1a2e3b] text-base font-bold text-white">
                                        {{ strtoupper(substr($user->username, 0, 1)) }}
                                    </div>
                                    <div class="min-w-0">
                                        <h3 class="truncate text-[.88rem] font-extrabold text-[#1a2e3b]">{{ $user->username }}
                    ...
                    @forelse($staffs as $user)
                        <article
                            ...
                            <div class="p-5">
                                <div class="mb-4 flex items-center gap-3">
                                    <div
                                        class="flex h-11 w-11 shrink-0 items-center justify-center bg-[#3d5a6e] text-base font-bold text-white">
                                        {{ strtoupper(substr($user->username, 0, 1)) }}
                                    </div>
                                    <div class="min-w-0">
                                        <h3 class="truncate text-[.88rem] font-extrabold text-[#1a2e3b]">{{ $user->username }}
