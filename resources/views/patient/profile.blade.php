        ...
        <section class="mt-6 grid gap-6 lg:grid-cols-3">
            <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
                <div class="flex items-center gap-4">
                    <div
                        class="h-14 w-14 rounded-full bg-slate-100 text-slate-700 flex items-center justify-center text-lg font-bold">
                        {{ $patientInitials }}
                    </div>
                    <div>
                        <p class="text-xs uppercase tracking-wide text-slate-400">Clinic Record ID</p>
                        ...
                        <p class="mt-1 text-xs text-slate-500">Clinic records are linked by staff after request
                            verification.</p>
                    </div>
                </div>
                ...
                <div class="mt-6 grid gap-4 sm:grid-cols-2">
                    ...
                    <div class="rounded-xl border border-slate-100 bg-slate-50 p-4">
                        <p class="text-xs uppercase tracking-wide text-slate-400">Account Email</p>
            ...
            <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div class="mt-4 space-y-3">
                    <div class="rounded-xl border border-slate-100 bg-slate-50 p-4">
                        ...
                        <p class="mt-1 text-sm font-semibold text-slate-900">
                            {{ $patient?->emergency_contact_name ?? 'N/A' }}</p>
                    </div>
                    <div class="rounded-xl border border-slate-100 bg-slate-50 p-4">
                        <p class="text-xs uppercase tracking-wide text-slate-400">Contact Number</p>
                        <p class="mt-1 text-sm font-semibold text-slate-900">
                            {{ $patient?->emergency_contact_number ?? 'N/A' }}</p>
                    </div>
                    <div class="rounded-xl border border-slate-100 bg-slate-50 p-4">
            ...
            <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                @else
                    <form action="{{ route('profile.password') }}" method="POST" class="mt-4 space-y-3">
                        ...
                        <div class="space-y-1">
                            <label class="text-xs font-semibold uppercase tracking-wide text-slate-500">Current
                                Password</label>
                        ...
                        <div class="space-y-1">
                            <label class="text-xs font-semibold uppercase tracking-wide text-slate-500">Confirm
                                Password</label>
