<div x-data="{ modalOpen: @entangle('showAppointmentModal').live, detailsLoading: false }"
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-4 h-[calc(100vh-280px)] min-h-[600px]">
        ...
        <div @if(!$showAppointmentModal && !$isPatientFormOpen) wire:poll.60s="refreshTodaySchedule" @endif class="flex flex-col min-h-0 overflow-hidden rounded-none border border-slate-200 bg-white shadow-sm">
            <div class="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-3.5">
                ...
                <span class="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-bold text-slate-700">{{ count($scheduledAppointments) }}</span>
            </div>
            ...
            <div class="flex-1 space-y-3 overflow-y-auto p-3.5 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-300">
                @if(count($scheduledAppointments) > 0)
                    @foreach($scheduledAppointments as $app)
                        <div wire:key="today-{{ $app->id }}-{{ $app->status }}" x-on:click="detailsLoading = true; modalOpen = true" wire:click="viewAppointment({{ $app->id }})" class="group cursor-pointer rounded-none border border-slate-200 bg-slate-50 p-4 transition hover:-translate-y-0.5 hover:border-slate-300 hover:bg-white hover:shadow-md">
                            ...
                            <div class="mt-3 flex items-center justify-between border-t border-slate-200 pt-3">
                                <span class="rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide bg-slate-200 text-slate-700">
                                    {{ $app->status }}
                                </span>
        ...
        <div @if(!$showAppointmentModal && !$isPatientFormOpen) wire:poll.10s="refreshWaitingQueue" @endif class="flex flex-col min-h-0 overflow-hidden rounded-none border border-amber-300 bg-amber-50/50 shadow-md">
                ...
                @if(count($waitingQueue) > 0)
                    @foreach($waitingQueue as $wait)
                        <div wire:key="wait-{{ $wait->id }}-{{ $wait->status }}" x-on:click="detailsLoading = true; modalOpen = true" wire:click="viewAppointment({{ $wait->id }})" class="group cursor-pointer rounded-none border bg-white p-4 transition hover:-translate-y-0.5 hover:border-amber-300 hover:shadow-md {{ $loop->first ? 'border-amber-300 ring-1 ring-amber-100 shadow-md' : 'border-amber-200 shadow-sm' }}">
                            ...
                            <div class="mt-3 rounded-none border border-amber-100 bg-amber-50 px-3 py-2 text-xs text-amber-900">
        ...
        <div @if(!$showAppointmentModal && !$isPatientFormOpen) wire:poll.5s="refreshOngoingAppointments" @endif class="flex flex-col min-h-0 overflow-hidden rounded-none border border-emerald-200 bg-white shadow-sm">
            ...
            <div class="flex-1 space-y-3 overflow-y-auto p-3.5 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-emerald-200">
                @if(count($ongoingAppointments) > 0)
                    @foreach($ongoingAppointments as $ongoing)
                        <div wire:key="ongoing-{{ $ongoing->id }}-{{ $ongoing->status }}" x-on:click="detailsLoading = true; modalOpen = true" wire:click="viewAppointment({{ $ongoing->id }})" class="group cursor-pointer rounded-none border border-emerald-200 bg-emerald-50/40 p-4 shadow-sm transition hover:-translate-y-0.5 hover:bg-white hover:shadow-md">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                            ...
                            <div class="mt-3 space-y-1 rounded-none border border-emerald-100 bg-white px-3 py-2 text-xs text-slate-600">
        ...
        <div @if(!$showAppointmentModal && !$isPatientFormOpen) wire:poll.60s="refreshTodaySchedule" @endif class="flex flex-col min-h-0 overflow-hidden rounded-none border border-rose-200 bg-white shadow-sm">
            <div class="flex items-center justify-between border-b border-rose-200 bg-rose-50 px-4 py-3.5">
                ...
                <span class="rounded-full border border-rose-200 bg-white px-2.5 py-1 text-[11px] font-bold text-rose-700">{{ count($cancelledAppointments) }}</span>
            </div>

            <div class="flex-1 space-y-3 overflow-y-auto p-3.5 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-rose-200">
                @if(count($cancelledAppointments) > 0)
                    @foreach($cancelledAppointments as $app)
                        <div wire:key="cancelled-{{ $app->id }}-{{ $app->status }}" x-on:click="detailsLoading = true; modalOpen = true" wire:click="viewAppointment({{ $app->id }})" class="group cursor-pointer rounded-none border border-rose-200 bg-rose-50/60 p-4 transition hover:-translate-y-0.5 hover:border-rose-300 hover:bg-white hover:shadow-md">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <h2 class="truncate text-sm font-bold text-slate-900">{{ $app->first_name }} {{ $app->last_name }}</h2>
                                    <p class="mt-0.5 truncate text-xs text-slate-500">{{ $app->service_name }}</p>
                                </div>
                                <span class="rounded-full border border-rose-200 bg-white px-2.5 py-1 text-[11px] font-bold text-rose-700 whitespace-nowrap">
                                    {{ \Carbon\Carbon::parse($app->appointment_date)->format('h:i A') }}
                                </span>
                            </div>

                            <div class="mt-3 flex items-center justify-between border-t border-rose-200 pt-3">
                                <span class="rounded-full bg-rose-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-rose-700">
                                    {{ $app->status }}
                                </span>
                                <span class="text-xs font-semibold text-slate-700 opacity-0 transition-opacity group-hover:opacity-100">View Details -&gt;</span>
                            </div>
                    ...
                    @endforeach
                @else
                    <div class="flex h-full flex-col items-center justify-center px-6 text-center text-sm text-rose-700">
                        <svg class="mb-2 h-10 w-10 text-rose-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M6 18L18 6M6 6l12 12"></path></svg>
    ...
    <div x-cloak x-show="modalOpen" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative z-10 mx-4 w-full max-w-4xl overflow-hidden rounded-none border border-slate-200 bg-white shadow-2xl">
                ...
                <div x-cloak x-show="!detailsLoading" class="max-h-[80vh] overflow-y-auto p-6">
                    ...
                    <div class="mb-5 grid grid-cols-1 gap-5 md:grid-cols-3">
                        <div>
                            ...
                            <input wire:model="firstName" type="text" class="w-full rounded-none border border-slate-200 bg-slate-50 px-4 py-2.5 font-medium text-slate-900 outline-none" readonly/>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-xs font-semibold text-slate-600">Middle Name</label>
                            <input wire:model="middleName" type="text" class="w-full rounded-none border border-slate-200 bg-slate-50 px-4 py-2.5 font-medium text-slate-900 outline-none" readonly/>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-xs font-semibold text-slate-600">Last Name</label>
                            <input wire:model="lastName" type="text" class="w-full rounded-none border border-slate-200 bg-slate-50 px-4 py-2.5 font-medium text-slate-900 outline-none" readonly/>
                        </div>
                    </div>
                    ...
                    <div class="mb-8 grid grid-cols-1 gap-5 md:grid-cols-2">
                        <div>
                            ...
                            <input wire:model="contactNumber" type="text" class="w-full rounded-none border border-slate-200 bg-slate-50 px-4 py-2.5 font-medium text-slate-900 outline-none" readonly/>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-xs font-semibold text-slate-600">Service Required</label>
                            <select wire:model="selectedService" class="w-full rounded-none border border-slate-200 bg-white px-4 py-2.5 font-medium text-slate-900 outline-none focus:ring-2 focus:ring-slate-200" {{ ($appointmentStatus != 'Waiting') ? 'disabled' : '' }}>
                    ...
                    <div class="flex flex-col justify-end gap-3 border-t border-slate-200 pt-6 sm:flex-row">
                        ...
                        @if(!in_array($appointmentStatus, ['Cancelled', 'Completed']))
                            <button type="button" wire:click="updateStatus('Cancelled')" wire:loading.attr="disabled" wire:target="updateStatus" wire:confirm="Are you sure you want to cancel this appointment?" class="mr-auto rounded-none px-5 py-2.5 font-semibold text-rose-600 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-60">
                                Cancel Appointment
                            </button>
                        @endif

                        @if($appointmentStatus === 'Scheduled')
                            <button type="button" wire:click="processPatient" wire:loading.attr="disabled" wire:target="processPatient" class="rounded-none border border-slate-200 bg-white px-5 py-2.5 font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">
                                Update Info
                            </button>
                            <button type="button" wire:click="updateStatus('Waiting')" wire:loading.attr="disabled" wire:target="updateStatus" class="flex items-center gap-2 rounded-none bg-amber-500 px-5 py-2.5 font-semibold text-white shadow-sm transition hover:bg-amber-600 disabled:cursor-not-allowed disabled:opacity-60">
                        ...
                        @elseif($appointmentStatus === 'Waiting')
                            ...
                            <button type="button" wire:click="admitPatient" wire:loading.attr="disabled" wire:target="admitPatient" class="flex items-center gap-2 rounded-none bg-emerald-600 px-5 py-2.5 font-semibold text-white shadow-sm transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                                Call to Chair
                            </button>

                        @elseif($appointmentStatus === 'Ongoing')
                            <button type="button" wire:click="openPatientChart" wire:loading.attr="disabled" wire:target="openPatientChart" class="flex items-center gap-2 rounded-none bg-slate-800 px-5 py-2.5 font-semibold text-white shadow-sm transition hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-60">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                Open Chart
                            </button>
                            <button type="button" wire:click="updateStatus('Completed')" wire:loading.attr="disabled" wire:target="updateStatus" class="flex items-center gap-2 rounded-none bg-emerald-600 px-5 py-2.5 font-semibold text-white shadow-sm transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                Finish Session
                            </button>

                        @elseif($appointmentStatus === 'Completed')
                            <span class="flex items-center gap-2 rounded-none border border-emerald-200 bg-emerald-50 px-5 py-2.5 font-bold text-emerald-700">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                Completed
                            </span>
                        @elseif($appointmentStatus === 'Cancelled')
                            <span class="flex items-center gap-2 rounded-none border border-rose-200 bg-rose-50 px-5 py-2.5 font-bold text-rose-700">
