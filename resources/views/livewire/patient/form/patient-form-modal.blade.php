        ...
        @endif
                        ...
                        @if ($currentStep == 4 && $isEditing && $canViewClinicalRecords)
                            <livewire:patient.form.treatment-record
                                ...
                                :imageUploadOnly="$treatmentImageUploadMode" />
                        @endif
                    ...
                    <div class="border-t border-gray-200 bg-white px-4 py-4 sm:px-6">
                        @php
                            ...
                            $showAddImagesButton = $isEditing
                                && $isReadOnly
                                && $isAdmin
                                && $currentStep === 4
                                && $hasTreatmentRecord
                                && ! $treatmentImageUploadMode;

                            $finalEditableStep = $isEditing
                                ? ($currentStep === 1 ? 1 : ($isAdmin ? 4 : 1))
                                : 1;
                            $shouldShowSave = ($treatmentImageUploadMode && $currentStep === 4)
                                || (! $isReadOnly && (! $isEditing || $currentStep === $finalEditableStep));
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div class="order-2 flex flex-wrap items-center gap-3 sm:order-1">
                                <button data-draft-ignore
                                    ...
                                    {{ (! $isReadOnly || $treatmentImageUploadMode) ? 'Cancel' : 'Close' }}
                                </button>
                            </div>
                            ...
                            <div class="order-1 flex flex-wrap items-center justify-end gap-3 sm:order-2">
                                ...
                                @if ($showAddImagesButton)
                                    <button wire:click="enableTreatmentImageUpload" type="button"
                                        class="inline-flex items-center gap-2 rounded-sm bg-[#0086da] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#0073a8]">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                            stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                ...
                                @if ($shouldShowSave)
                                    <button data-draft-ignore
                                        ...
                                        @if ($treatmentImageUploadMode && $currentStep === 4)
                                            Save Images
                                        @elseif (!$isEditing)
                                            Register Patient
                                        @else
