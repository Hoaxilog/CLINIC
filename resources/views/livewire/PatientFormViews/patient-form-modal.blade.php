<div>
    @if ($showModal)
        <div data-patient-form-modal class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-3 sm:p-6"
            ...
            x-init="init()"
            x-on:patient-form-opened.window="handleModalOpened()"
            ...
            x-on:patient-added.window="handleSuccessfulSave()"
            x-on:input.capture="markDirtyDebounced()"
            x-on:change.capture="markDirtyDebounced()"
            x-on:click.capture="handleClickCapture($event)">
            ...
            <div class="w-full max-w-[108rem] overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-2xl">
                ...
                <div class="flex max-h-[92vh] flex-col">
                    ...
                    <div class="border-b border-slate-200 bg-white px-4 py-4 sm:px-6">
                        ...
                        <div x-cloak x-show="prompt.visible"
                            ...
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                <div class="flex items-center gap-2">
                                    <button type="button" data-draft-ignore @click="$dispatch('patient-form-draft-pause', { ms: 2500 }); restoreDraft()" :disabled="restoring"
                                    ...
                                    <button type="button" data-draft-ignore @click="$dispatch('patient-form-draft-pause', { ms: 2500 }); discardDraft()" :disabled="restoring"
                    ...
                    <div class="border-t border-slate-200 bg-white px-4 py-4 sm:px-6">
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div class="order-2 flex flex-wrap items-center gap-3 sm:order-1">
                                <button data-draft-ignore x-on:click="$dispatch('patient-form-draft-pause', { ms: 3000 })" wire:click="cancelEdit" type="button" class="rounded-lg border border-slate-300 bg-white px-5 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50">
                            ...
                            <div class="order-1 flex flex-wrap items-center justify-end gap-3 sm:order-2">
                                ...
                                @if($shouldShowSave)
                                    <button data-draft-ignore x-on:click="$dispatch('patient-form-draft-pause', { ms: 3000 })" data-trigger-scroll wire:click="save" type="button"
                ...
                window.patientFormDraftManager = function (config) {
                    return {
                        ...
                        buildDraftPayload() {
                            ...
                            const treatmentFields = ['dmd', 'treatment', 'cost_of_treatment', 'amount_charged', 'remarks'];
                            ...
                            for (const btn of toggles) {
                                if (btn.classList.contains('bg-sky-600') && btn.textContent.trim().toLowerCase() === 'child') {
                                    dentitionType = 'child';
                                }
                            }
                        ...
                        detectCurrentStep() {
                            ...
                            if (modal.querySelector('[wire\\:model\\.defer="dmd"], [wire\\:model\\.defer="treatment"]')) {
                                return 4;
                            }

                            // Step 3 marker (dental chart)
                            if (modal.querySelector('[wire\\:model\\.defer="oralExam.gingiva"], [wire\\:model\\.defer="oralExam.oral_hygiene_status"]')) {
                                return 3;
                            }
